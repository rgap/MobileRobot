...
void CanRecognition::shapeSegmentation(Mat& imgSegColor_Can, Mat& imgSegShape_Can,
	...
	for (i = 0; i < contours.size(); i++) {
		...
		if (area >= minArea && area <= maxArea && apCuadrado >= minApCuadrado
			...
			rmin = MAX_DEPTH;
			for (k = boundRect[i].tl().y; k <= boundRect[i].br().y; k++) {
				uchar*imgSegColor_Can_pt = imgSegColor_Can.ptr<uchar> (k);
				uchar*imgMaxDepth_pt = imgMaxDepth.ptr<uchar> (k);

				for (j = boundRect[i].tl().x; j <= boundRect[i].br().x; j++) {
					if (imgMaxDepth_pt[j] == 255 && imgSegColor_Can_pt[j]
							== 255) { // PARTE DE LA LATA
						z = world.at<Vec3f> (k, j)[2];
						if (z < rmin)
							rmin = z;
					}
				}
